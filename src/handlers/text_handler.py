...
class TextHandler:
    ...
    def _handle_group_message(self, message):
        ...
        if f'@{bot_username}' in message.text:
            ...
            self._answer_query(message, question)
        ...
        # Check for reply to bot message
            ...
            self._answer_query(message, message.text)
        else:
    ...
    def _answer_query(self, message, question):
        ...
        try:
            ...
            response = self.ai_client.query_ai(question, full_context)
            ...
            self._store_bot_response(message, html_response)
            
            # Send response
            self._send_response(message, html_response)
            
        except Exception as e:
            logger.error(f"Error during response generation: {e}")
            error_response = "Sorry, there was a problem processing your request."
            self._send_response(message, error_response)
    
    def _store_bot_response(self, message, response):
        ...
        response_message = {
            ...
            'from': {
                ...
                'is_bot': True,
                'first_name': self.bot.get_me().first_name,
    ...
    def _send_response(self, message, response):
        ...
        if len(response) > 4096:
            for i in range(0, len(response), 4096):
                ...
                if message.chat.type == 'private':
                    self.bot.send_message(message.chat.id, chunk, parse_mode=parse_mode)
                else:
                    self.bot.reply_to(message, chunk, parse_mode=parse_mode)
        else:
            if message.chat.type == 'private':
                self.bot.send_message(message.chat.id, response, parse_mode=parse_mode)
            else:
                self.bot.reply_to(message, response, parse_mode=parse_mode)
